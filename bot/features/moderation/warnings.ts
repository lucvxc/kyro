...
import type { WarnPunishment } from "../../db/settings.ts";
...
export async function warn(
  ...
  const punishment = (await punishments(guildId)).find(
    (value) => value.warnings === records.length,
  );
...
export async function punishments(guildId: string): Promise<WarnPunishment[]> {
  const [guild] = await db
    .select({ value: guilds.warnPunishments })
    .from(guilds)
...
export async function setPunishment(
  ...
  const next =
    typeof value === "number"
      ...
      : [...current.filter((item) => item.warnings !== count), value].sort(
          (a, b) => a.warnings - b.warnings,
        );

  await db
    .insert(guilds)
    .values({ id: guildId, warnPunishments: next })
    .onConflictDoUpdate({
      target: guilds.id,
      set: { warnPunishments: next, updatedAt: new Date() },
    });
