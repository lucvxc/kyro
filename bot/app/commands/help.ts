...
function home(
  ...
  return container()
    ...
    .separator()
    .text("Select a category to view every command.")
    .row(picker);
}
...
function category(
  ...
  const commands = found.roots
    .map((root) => {
      const subs = found.commands
        .filter(
          (command) => command.path[0] === root && command.path.length > 1,
        )
        .map((command) => command.path.slice(1).join(" "));
      return subs.length ? `${root}\n  ${subs.join(", ")}` : root;
    })
    .join("\n");
