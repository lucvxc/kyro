...
export default cmp({
  ...
  run: async (ctx) => {
    ...
    if (action === "embedmgr") {
      ...
      return ctx.private(embedDetails(await savedEmbed(ctx.user.id, selected)));
    }
    ...
    if (action === "embededitsave") {
      ...
      return ctx.private(
        embedDetails(await updateEmbedCode(ctx.user.id, key, code)),
      );
    }

    if (action === "embedback") {
      return ctx.update(embedDetails(await savedEmbed(ctx.user.id, key)));
    }
...
function embedDetails(saved: SavedEmbed) {
  return container()
