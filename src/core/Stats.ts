...
export class Stats {
  ...
  public get bot(): ClientUser {
    if (!this.client.user) throw new Error("The bot is not ready.");
  ...
  public get ping(): number {
    return Math.max(0, Math.round(this.client.ws.ping));
  }
  public get memory(): number {
    return Math.round(process.memoryUsage().rss / 1_048_576);
  }
  public get since(): number {
    return Math.floor((this.client.readyTimestamp ?? Date.now()) / 1_000);
  }
