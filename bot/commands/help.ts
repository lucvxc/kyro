import { cmd, container, dominant, select, thumb, UserError } from "../../index.ts";
...
export default cmd({
  ...
  run: async (ctx) => {
    ...
    collector.on("collect", async interaction => {
      ...
      await ctx.update(interaction, selected === "home"
...
function home(ctx: Context, catalog: Catalog, picker: ReturnType<typeof select>, avatar: string, accent: string) {
  return container()
    .accent(accent)
    .section(`## ${ctx.stats.name} Help\n-# Browse ${catalog.commands.length} commands across ${catalog.categories.length} categories.`, thumb(avatar))
    .separator()
    .text("`*` has subcommands")
    .row(picker);
}

function command(catalog: Catalog, query: string, prefix: string, accent: string) {
  const found = catalog.find(query);
  const subs = catalog.subs(found ?? query);
  if (!found && !subs.length) throw new UserError(`No command found for "${query}".`);
  ...
  if (!found) {
    return container()
      ...
      .separator()
      .text(subs.map(item => `**${prefix}${item.syntax}**\n-# ${item.description}`).join("\n\n"));
  }
  ...
  const card = container()
    ...
    .text([
      ...
      found.aliases.length ? `**Aliases** ${found.aliases.map(alias => `\`${alias}\``).join(" · ")}` : "",
    ].filter(Boolean).join("\n"));

  if (subs.length) card.text(`**Subcommands**\n${subs.map(item => `\`${item.name}\``).join(" · ")}`);
  return card;
}

function category(catalog: Catalog, name: string, picker: ReturnType<typeof select>, accent: string) {
