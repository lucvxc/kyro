...
import { lockGuild, unlockGuild } from "../../../features/security/lockdown.ts";
...
import type { AntiRaidSettings, RaidAction } from "../../../db/settings.ts";
...
export default cmp({
  ...
  run: async (ctx) => {
    ...
    const settings = (await securitySettings(ctx.guild!.id)).antiraid;
    ...
    if (action === "lockdown") {
      if (settings.lockdown.active) await unlockGuild(ctx.guild!);
...
function open(
  ...
  const item = settings.protections[name as Name];
