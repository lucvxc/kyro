...
import { removeAlias } from "../../../services/settings/commands.ts";
...
export default cmd({
  ...
  args: { alias: { type: "string", required: true, description: "Custom alias" } },
  run: async ctx => {
    const alias = ctx.string("alias")!.trim().toLowerCase();
    if (!await removeAlias(ctx.guild!.id, alias)) throw new UserError(`The **${alias}** alias does not exist.`);
