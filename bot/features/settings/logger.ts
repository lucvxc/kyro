...
import { Cache, send, type Container } from "../../../index.ts";
import { db } from "../../db/database.ts";
import { guilds } from "../../db/schema.ts";
import type { LogSettings } from "../../db/settings.ts";
...
export async function setLoggerEnabled(
  guildId: string,
  enabled: boolean,
): Promise<void> {
  const current = await loggerSettings(guildId);
...
export async function setLoggerChannel(
  guildId: string,
  channelId: string,
): Promise<void> {
  const current = await loggerSettings(guildId);
...
export async function setLogEvent(
  guildId: string,
  event: LogEvent,
  enabled: boolean,
): Promise<boolean> {
  ...
  const events = enabled
    ? [...active, event]
    : active.filter((value) => value !== event);
  await save(guildId, { ...current, events });
  return true;
}

export async function setIgnoredChannel(
  guildId: string,
  channelId: string,
  ignored: boolean,
): Promise<boolean> {
  ...
  const ignoredChannels = ignored
    ? [...channels, channelId]
    : channels.filter((value) => value !== channelId);
...
export async function sendLog(
  ...
  if (sourceChannelId && config.ignoredChannels?.includes(sourceChannelId))
    return false;
...
async function save(guildId: string, logging: LogSettings): Promise<void> {
  ...
  await db
    .insert(guilds)
    .values({ id: guildId, logging: value })
    .onConflictDoUpdate({
      target: guilds.id,
      set: { logging: value, updatedAt: new Date() },
    });
