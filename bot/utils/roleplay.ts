...
export function target(name: string, action: string, after = "", endpoint = name) {
  const phrase = `${action} someone${after ? ` ${after}` : ""}`;
  return make(name, phrase, ctx => {
    const user = ctx.user("user") ?? "themselves";
    return `${ctx.author} ${action} ${user}${after ? ` ${after}` : ""}`;
  }, true, endpoint);
}

function make(name: string, description: string, text: (ctx: Context) => string, targeted = false, endpoint = name) {
  return cmd({
    name,
    ...
    args: targeted ? { user: { type: "user", description: "The user to interact with" } } : undefined,
    run: ctx => send(ctx, endpoint, text(ctx)),
  });
}

async function send(ctx: Context, endpoint: string, text: string): Promise<void> {
