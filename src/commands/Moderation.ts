...
export class Moderation {
  ...
  public async untimeout(user: User, options: ModOptions = {}): Promise<void> {
    ...
    if (!member.isCommunicationDisabled())
      throw new UserError("That member is not timed out.");
    await member.timeout(null, this.#reason(options));
  }

  public async unban(user: User, options: ModOptions = {}): Promise<void> {
    const guild = this.#guild();
    if (!(await guild.bans.fetch(user.id).catch(() => null)))
      throw new UserError("That user is not banned.");
  ...
  public async strip(user: User, options: ModOptions = {}): Promise<Role[]> {
    ...
    if (!roles.length)
      throw new UserError("That member has no roles I can remove.");
  ...
  public async stripRole(
    ...
    const members = [...role.members.values()].filter(
      (member) =>
    ...
    await Promise.all(
      members.map((member) => member.roles.remove(role, this.#reason(options))),
    );
  ...
  async #member(guild: Guild, user: User): Promise<GuildMember> {
    return guild.members.fetch(user.id).catch(() => {
  ...
  async #voice(
    ...
    if (!member.voice.channelId)
      throw new UserError("That member is not in a voice channel.");
    return member;
  }
  async #check(
    member: GuildMember,
  ...
  #reason(options: ModOptions): string {
    return (
