...
export default cmd({
  name: "banlist", aliases: ["bans"], description: "Show users banned from this server.", type: "message", context: "guild",
  permissions: [PermissionFlagsBits.BanMembers],
  run: async ctx => {
    const bans = await ctx.guild!.bans.fetch({ limit: 100 });
    const list = [...bans.values()].slice(0, 25).map(ban => `**${ban.user.tag}** · ${ban.user.id}`).join("\n");
