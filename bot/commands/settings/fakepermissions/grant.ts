...
export default cmd({
  ...
  args: {
    role: { type: "role", required: true, description: "Role receiving the permission" },
    permission: { type: "string", required: true, autocomplete: true, description: "Discord permission to grant" },
  },
  ...
  run: async ctx => {
    ...
    return ctx.reply(added
      ...
      : embeds.warning(`<@&${role.id}> already has **${label(permission)}**.`));
  },
});
