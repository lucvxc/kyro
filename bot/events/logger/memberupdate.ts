...
export default evt({
  ...
  run: async (before, after) => {
    if (before.communicationDisabledUntilTimestamp !== after.communicationDisabledUntilTimestamp) {
      ...
      const actor = await auditActor(after.guild, AuditLogEvent.MemberUpdate, after.id);
      await sendLog(after.guild, "memberTimeout", loggerCards.timeout(after, removed, actor));
    }
    ...
    if (added.length || removed.length) {
      ...
      await sendLog(after.guild, "memberRoles", loggerCards.roles(after, added, removed, actor));
    }
  },
});
