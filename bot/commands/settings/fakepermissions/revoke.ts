...
export default cmd({
  ...
  args: {
    role: {
      type: "role",
      required: true,
      description: "Role losing the permission",
    },
    permission: {
      type: "string",
      required: true,
      autocomplete: true,
      description: "Discord permission to revoke",
    },
  },
  ...
  run: async (ctx) => {
    ...
    return ctx.reply(
      removed
        ? embeds.success(
            `Revoked **${label(permission)}** from <@&${role.id}>.`,
          )
        : embeds.warning(
            `<@&${role.id}> does not have **${label(permission)}**.`,
          ),
    );
  },
});
