...
export async function toggleSticky(guildId: string): Promise<boolean> {
  const [guild] = await db
    .select({ enabled: guilds.stickyRoles })
    .from(guilds)
  ...
  await db
    .insert(guilds)
    .values({ id: guildId, stickyRoles: enabled })
    .onConflictDoUpdate({
      target: guilds.id,
      set: { stickyRoles: enabled, updatedAt: new Date() },
    });
  if (!enabled)
    await db.delete(stickyMembers).where(eq(stickyMembers.guildId, guildId));
  return enabled;
}
...
export async function rememberRoles(member: GuildMember): Promise<void> {
  ...
  await db.transaction(async (tx) => {
    ...
    if (roleIDs.length) {
      await tx.insert(stickyMembers).values(
        roleIDs.map((roleId) => ({
...
export async function restoreRoles(member: GuildMember): Promise<void> {
  ...
  await new Server(member.guild).roles.restore(
    member.id,
    rows.map((row) => row.roleId),
    "Restoring sticky roles",
  );
  if (rows.length)
    await db
      .delete(stickyMembers)
