...
import { fakePerms, label, owner, permissions } from "../../../utils/fakepermissions.ts";
...
export default cmd({
  ...
  run: async ctx => {
    ...
    const assigned = groups(grants, grant => grant.roleId, (roleID, values) =>
      `${mention.role(roleID)}\n${codes(values, value => label(value.permission))}`) || (role
        ? `<@&${role.id}> has no fake permissions.`
        : "No fake permissions have been assigned yet.");

    return ctx.reply(embeds.default(`### Available\n${available}\n\n### Assigned\n${assigned}`, {
      title: role ? `Fake Permissions · ${role.name}` : "Fake Permissions",
      footer: { text: `${permissions.length} available · ${grants.length} assigned` },
    }));
  },
});
