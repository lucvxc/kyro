...
export async function addTempRole(
  ...
  await db
    .insert(tempRoles)
    .values({ guildId, userId, roleId, expiresAt })
    .onConflictDoUpdate({
...
function schedule(client: Client, role: typeof tempRoles.$inferSelect): void {
  ...
  const delay = Math.min(
    Math.max(0, role.expiresAt.getTime() - Date.now()),
    2_147_000_000,
  );
...
async function expire(
  client: Client,
  role: typeof tempRoles.$inferSelect,
): Promise<void> {
  const key = `${role.guildId}:${role.userId}:${role.roleId}`;
  ...
  if (member?.roles.cache.has(role.roleId)) {
    await member.roles
      .remove(role.roleId, "Temporary role expired")
      .catch(() => undefined);
  }
  await db
    .delete(tempRoles)
