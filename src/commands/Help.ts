...
export function help(registry: Registry): Cmd {
  return {
    ...
    run: (ctx) => {
      ...
      if (query && !selected)
        return ctx.reply(`I couldn't find a command named "${query}".`);
      ...
      if (selected) {
        const args = Object.entries(selected.args ?? {})
          .map(
            ([name, arg]) =>
              `• \`${name}\` - ${arg.description ?? "No description"}${arg.required ? " **required**" : ""}`,
          )
          .join("\n");
        const usage = `\`/${selected.name}${Object.keys(selected.args ?? {})
          .map((name) => ` <${name}>`)
          .join("")}\``;
        ...
        if (selected.aliases.length)
          card.field(
            "Aliases",
            selected.aliases.map((alias) => `\`${alias}\``).join("  ·  "),
          );
      } else {
        const list = commands
          .slice(0, 25)
